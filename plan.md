...
- **Save / Save As**: Implement native "Save" and "Save As" functionality using Tauri's `@tauri-apps/plugin-fs`.
- **Title Bar Integration**: Display the current filename in the window title bar, with an asterisk (*) indicating unsaved changes.

### Toolbar
- A minimal toolbar at the top with buttons for formatting:
  `B` (Bold), `I` (Italic), `H` (Heading), `List`, `Link`.
- Keyboard shortcuts:
  `Cmd/Ctrl + S` (Save),
  `Cmd/Ctrl + O` (Open),
  `Cmd/Ctrl + N` (New).
...
- **Frontend**: Vite with **Vue.js 3** (Composition API). In component files, the `Template` block should come first.
- **State Management**: Pinia.
- **Styling**: Vanilla CSS.
- **Libraries**:
  - `marked.js` for Markdown parsing.
  - `prism.js` for code syntax highlighting
- **Packaging**: Ensure the app can be build for macOS, Windows, Linux

## Project Structure
- Standard Tauri 2 project structure.
- Frontend logic should handle the editor state and communication with the rust backend if needed
